...
from .puzzlesolver.solvers import *
import math

class Test(ServerPuzzle):
    ...
    def __init__(self, size=3):
        ...
        self.position = Test.swap(self.position, self.size**2 -1, self.size**2-2)
    ...
    def doMove(self, move):
        newPuzzle = Test(size=self.size)
        newPuzzle.position = Test.swap(self.position[:], move[0], move[1])
        return newPuzzle
    ...
    def generateStartPosition(cls, variantid, **kwargs):
        ...
        if variantid not in Test.variants: raise IndexError("Out of bounds variantid")
        return Test(size=int(variantid))
