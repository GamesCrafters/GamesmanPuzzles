...
int Hanoi_init(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (kwds != NULL) {
        PyObject* variant = PyDict_GetItemString(kwds, "variant");
        if (!PyDict_Check(variant)) { 
            PyErr_SetString(PyExc_TypeError, "Invalid arguments");
            return -1;
        }
        PyObject* py_disk = PyDict_GetItemString(variant, "disk_variant");
        PyObject* py_rod = PyDict_GetItemString(variant, "rod_variant");
        if (py_disk) {
            if (PyLong_Check(py_disk)) {
...
int PyModule_AddHanoi(PyObject* module) {
    ...
    name = PyUnicode_FromString("Anthony Ling");
    puzzleid = PyUnicode_FromString("hanoi");
    puzzle_name = PyUnicode_FromString("Tower of Hanoi");
    description = PyUnicode_FromString(
        "Move smaller discs ontop of bigger discs. Fill the rightmost stack."
    );
    date_created = PyUnicode_FromString("April 2, 2020");

    PyDict_SetItemString(dict, "author", name);
