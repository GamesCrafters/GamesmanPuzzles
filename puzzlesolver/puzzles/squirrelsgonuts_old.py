...
from ..solvers import NoUndoSolver
import random
import os
...
class Squirrels(ServerPuzzle):
    ...
    def __init__(self, variant_id="starter", puzzle_id=None, pos=None, squirrels_list=None, nuts_list=None, nuts_left=None, hole_list=None, actual_hole_list=None, hole_matching=None):
        """
    ...
    def __hash__(self):
        ...
        #print(tuple(sorted((k, tuple(v)) for k, v in self.squirrels_list.items())))
        return hash((
        tuple(self.pos), 
        ...
        #return hash((tuple(self.pos), tuple(self.hole_matching.items())))
        
        #return hash(tuple(self.pos)) # CHECK THIS - USE THIS FOR HASH MAYBE?

    def _toString(self, mode):
        """
        Inputs:
    ...
    def doMove(self, move, **kwargs):
        ...
        for piece in move:
            ...
            if end in self.hole_coords and self.nuts_list[type_index] in self.nuts_left and relation == "O": # CHECK THIS UPDATE: now checks if hole and if still usable hole for nuts -> past code (self.pos[end] in self.hole_matching.values())
                new_actual_hole_list[end] = type + "_" + type_index # hole location : nut index
                #print(self.nuts_left)
                #print(self.nuts_list)
                #print(type_index)
                #print("_____________")
                #print(self.nuts_list[type_index])
                #print(piece)
    ...
    def toString(self, mode):
        ...
        for i, piece in enumerate(self.pos):
            ...
            if piece[0] == 'B':
                outlist[i] = 'X'
            if piece[:3] in self.actual_hole_list.values():
                outlist[i] = 'a'
        return ''.join(outlist)
