...
class NoUndoSolver(Solver):
    ...
    def getRemoteness(self, puzzle):
        ...
        if not isinstance(puzzle, Puzzle):
            raise TypeError("Not a Puzzle instance")
        if puzzle.primitive() == PuzzleValue.UNSOLVABLE:
    ...
    def solve(self, verbose=False):
        ...
        while not self._queue.empty():
            ...
            for nextPuzzle in self.parent_map.get(hash(puzzle)):
                if hash(nextPuzzle) not in self._remoteness:
                    ...
                    h_new = hash(nextPuzzle)
                    self._remoteness[h_new] = self._remoteness[h] + 1
    ...
    def _cspGenerateSolutions(self, queue, verbose=False):
        ...
        while not queue_2.empty():
            ...
            for move in puzzle.generateMoves('legal'):
                ...
                h = hash(nextPuzzle)
                if not self.parent_map.get(h):
                    self.parent_map[h] = set()
                self.parent_map[h].add(puzzle)
                if h not in found:
                    found.add(h)
